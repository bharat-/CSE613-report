...
We took the aggregated reading on all available cores of both Socket 0
\& 1.  We compared numactl with -i and -m option, and observed that in
general both of them had same performance in terms of energy, but -i
gives better cache performance, hence we used -i in our experiment.
We have used CILKNWORKERS=16
\emph{numactl} memory and cpu interleaving ~\cite{NUMA-MAN}.

To remove anamolies from experimental readings, we ran the experiment 7
